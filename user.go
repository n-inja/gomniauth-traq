...
const (
	traqKeyID          string = "userId"
	traqKeyName        string = "name"
	traqKeyDisplayName string = "displayName"
	traqKeyIconFileId  string = "iconFileId"
	traqUserIconPath   string = "https://traq-dev.tokyotech.org/api/1.0/files/"
)
...
// NewUser builds a new User object for traQ.
func NewUser(data objx.Map, creds *common.Credentials, provider common.Provider) *User {
	user := &User{data}

	creds.Set(common.CredentialsKeyID, data[traqKeyID])
...
func (u *User) Name() string {
	return u.Data().Get(traqKeyName).Str()

}

// NickName gets the users nickname or username.
func (u *User) Nickname() string {
	return u.Data().Get(traqKeyDisplayName).Str()
}

// AvatarURL gets the URL of an image representing the user.
func (u *User) AvatarURL() string {
	return traqUserIconPath +
		u.Data().Get(traqKeyIconFileId).Str()
}
